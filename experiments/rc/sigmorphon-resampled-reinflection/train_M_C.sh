...
#SBATCH --output=logs/train_sigmorphon_resampled-reinflection_M_C_inflector.%j.log
...
readonly TRAIN="${DATA}/train/${PARTITION}/${LANGUAGE}_sampled_sigmorphon_resampling_reinflection.tsv"
readonly DEV="${DATA}/test/${LANGUAGE}-dev"
readonly RESULT="${ROOT}/results/${PARTITION}/${K}/M_C-sigmorphon-resampled-reinflection/${LANGUAGE}"

cd $LIB
# Hyperparams from 2018 IL paper
python -u run_transducer.py --dynet-seed $SEED --random-seed $SEED --dynet-mem 700 \
